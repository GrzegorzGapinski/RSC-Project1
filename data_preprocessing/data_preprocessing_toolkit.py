...
class DataPreprocessingToolkit(object):
    ...
    def filter_out_long_stays(df):
        ...
        # Write your code here #
        ########################
        condition = df['length_of_stay'] < 22
        filtered_df = df.loc[condition]
        return filtered_df
    ...
    def filter_out_low_prices(df):
        ...
        # Write your code here #
        ########################
        condition = df['accommodation_price'] > 50
        filtered_df = df.loc[condition]
        return filtered_df
    ...
    def add_length_of_stay(df):
        ...
        # Write your code here #
        ########################
        df['length_of_stay'] = (pd.to_datetime(df['date_to']) - pd.to_datetime(df['date_from'])).dt.days
        return df
    ...
    def add_book_to_arrival(df):
        ...
        # Write your code here #
        ########################
        df['book_to_arrival'] = (pd.to_datetime(df['date_from']) - pd.to_datetime(df['booking_date'])).dt.days
        return df
    ...
    def add_weekend_stay(df):
        ...
        for idx, row in df.iterrows():
            los = pd.to_datetime(row['date_to']) - pd.to_datetime(row['date_from'])
            checker = False
            if (los.days >= 5):
                #row['weekend_stay'] = "True"
                weekend_values.append("True")
                checker = True
            else:
                #date = row['date_from']
                for i in range(los.days + 1):
                    date = row['date_from'] + timedelta(days=i)
                    if (date.dayofweek == 4) or (date.dayofweek == 5):
                        #row['weekend_stay'] = "True"
                        checker = True
                if not checker:
                    #row['weekend_stay'] = "False"
                    weekend_values.append("False")
                else:
                    weekend_values.append("True")
        
        #print(len(weekend_values))
        #print(len(df))
        df['weekend_stay'] = weekend_values

        #df['weekend_stay'] = "True"
        return df
    ...
    def aggregate_group_reservations(self, df):
        ...
        # Write your code here #
        ########################
        grouped_res_id = group_reservations[['reservation_id', 'group_id']]
        
        gr_grouped_sum = group_reservations.groupby('group_id')[self.sum_columns].sum()
        gr_grouped_mean = group_reservations.groupby('group_id')[self.mean_columns].mean()
        gr_grouped_mode = group_reservations.groupby('group_id')[self.mode_columns].agg(pd.Series.mode)
        gr_grouped_first = group_reservations.groupby('group_id')[self.first_columns].first()
        """
        display(gr_grouped_sum.head(10))
        ...
        #group_reservations_merged = pd.concat([group_reservations_merged, grouped_res_id], axis = 1)
        #group_reservations_merged = pd.merge(group_reservations_merged, grouped_res_id, on='group_id', how='left')
        ...
        #df.sort_values(df['reservation_id'])
    ...
    def map_night_prices_to_room_segment_buckets(self, df):
        ...
        ########################
        df_copy = df.copy()
        df_copy = df_copy.groupby('room_group_id')['night_price'].mean()
        #display(df_copy.head(10))
        rgi_dict = df_copy.to_dict()
        #display(rgi_dict)
        a_values = []
        for idx, row in df.iterrows():
            a_values.append(rgi_dict[row['room_group_id']])
        #df['avg_night_price'] = rgi_dict[df['room_group_id']]
        df['avg_night_price'] = a_values
        #display(df.head(10))
        
        df['room_segment'] = df['avg_night_price'].apply(lambda x: self.map_value_to_bucket(x, self.room_segment_buckets))
        return df
